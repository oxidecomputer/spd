...
//! EE1004-v 4 Kbit serial presence detect (SPD), used in DDR4
...
impl Function {
    /// For a given function, return its device code.
    ///
    /// This code follows the structure of Table 2 in EE1004.
    pub fn to_device_code(self) -> Option<u8> {
        match self {
            ...
            Function::PageAddress(page) => {
                ...
                match page {
                    Page::Page0 => Some(dtid | 0b110),
                    Page::Page1 => Some(dtid | 0b111),
                }
            }
        }
    }

    /// For a given device code, return the function (if any).
    ///
    /// This code follows the structure of Table 2 in EE1004, which yes, is in
    /// function order not device code order.
    pub fn from_device_code(code: u8) -> Option<Self> {
        ...
        match device_type_identifier {
            ...
            0b0110 => match select_address {
                ...
                0b110 => Some(Function::PageAddress(Page::Page0)),
                0b111 => Some(Function::PageAddress(Page::Page1)),
                _ => None,
            },
            _ => None,
        }
